...
def test_download_main_file_uses_prepared_download_url(monkeypatch):
    api, _ = make_api()
    calls = []

    monkeypatch.setattr(
        api,
        "_prepare_product_main_file_download",
        lambda product_id: {
            "status": "ready",
            "download_url": "/api/products/42/download/main-file/file/?token=signed",
        },
    )
    monkeypatch.setattr(
        api,
        "_download_request",
        lambda url, save_in: calls.append((url, save_in))
        or {"success": True, "message": f"{save_in}/main_file.csv"},
    )

    result = api.download_main_file(42, "/tmp")

    assert result["success"] is True
    assert calls == [
        (
            "https://pz.example.org/api/products/42/download/main-file/file/?token=signed",
            "/tmp",
        )
    ]


def test_download_main_file_polls_until_ready(monkeypatch):
    api, communicate = make_api()
    statuses = [
        {"status": "running"},
        {
    ...
    monkeypatch.setattr(
        api,
        "_get_product_main_file_download_status",
        lambda product_id: statuses.pop(0),
    )
    monkeypatch.setattr(
        api,
    ...
    result = api.download_main_file(42, "/tmp", timeout=10, poll_interval=0)

    assert result["success"] is True
    assert not statuses


def test_download_main_file_raises_when_archive_failed(monkeypatch):
    api, _ = make_api()
