...
class PzServer:
    ...
    def get_product_metadata(self, product_id=None, show_main_file_info=False):
        ...
        if "_" in product_id:
            ...
            if show_main_file_info:
                return self.api.get("products", product_id, "main_file")
    ...
    def get_product(self, product_id=None):
        ...
        # ver formato, header, etc
        main_file_dict = self.get_product_metadata(product_id,
                                                   show_main_file_info=True)['main_file']
        os.system("mkdir .tmp")
        # baixar como oculto ".temp_file.csv"  criar diretório oculto
        self.api.download_product(
            product_id, save_in=".tmp", main_file_only=True)
        if main_file_dict['type'] == 'text/csv':
            data = ascii.read(
                main_file_dict['name'], format='csv', fast_reader=False)
        os.system("rm -rf .tmp")
        return data
        # abrir arquivo
        # dataframe ou Astropy table
        # deletar arquivo
        # return dataframe
    ...
    def download_product(self, product=None, save_in="."):
        ...
        """
        result_dict = self.api.download_product(product, save_in)
