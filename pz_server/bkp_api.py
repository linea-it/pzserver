...
class PzServerApi:
    ...
    def get(self, entity, _id, func=""):
        ...
        return self._get_request(f"{self._base_api_url}{entity}/{_id}/{func}/")

    def download_product(self, _id, save_in=".", main_file_only=False):
        """ Download data product to local 

        Args:
            _id (int): record id
            save_in (str): location where the file will be saved
            main_file_only: if False (default), downloads all 
                            files in one zip file. If True, downloads
                            only the main file in original format.   

        """
        if main_file_only:
            return self._download_request(f"{self._base_api_url}products/{_id}/content/", save_in)
